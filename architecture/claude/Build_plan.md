...
25. ✅ NL research assistant — built and verified live 2026-08-30, see §X.8.
...
### X.8 NL Research Assistant *(P2 step 25 — built 2026-08-30)*

- **Purpose:** answer Screener.md §18's example questions in plain language — "why did this stock fall?", "compare with competitors", "has this happened before?", "what would invalidate my thesis?", "find companies where the market may be overreacting", "summarize my portfolio" — by letting the model call read-only tools into this app's own already-tested services, never inventing a number.
- **Blocked, then unblocked, by the same saga as step 19's runtime.** Both of §U.12's bugs (query-param auth, then `gemini-flash-latest` overload) looked from the outside like "tool calling doesn't work on this key" — verified live neither was: native function calling round-trips correctly and quickly on `gemini-flash-lite-latest` with the same keys, once both bugs were actually fixed.
- **Architecture:** `app/providers/ai/gemini_chat.py` is a new, separate provider from `gemini_summary.py` — one-shot text generation and a multi-turn tool-calling step are different request/response shapes, and reuses only the model-fallback constants, not the retry loop (each has its own reasons, documented in each module). `app/services/research_assistant.py` owns the tool registry, the system instruction, and the orchestration loop: send the conversation + tool declarations, dispatch a `functionCall` to the matching service wrapper, feed the result back as a `functionResponse`, repeat until the model answers in plain text or `MAX_TOOL_CALLS` (6) is exhausted.
- **13 tools**, each a thin wrapper over an existing service — company overview, Opportunity Score, technicals, fundamentals, historical falls, news, corporate actions, peer-percentile comparison (reusing §X.4's exact numbers), a named screen, and four user-scoped ones (portfolio, watchlist, theses list, one thesis in full). The three user-scoped tools take `current_user` from the authenticated request, never as a model-supplied argument — there is no `user_id` field in any tool's JSON schema for the model to fill in, correctly or by injection.
- **Grounded by construction:** the system instruction tells the model it has no knowledge beyond what a tool just returned in this conversation, and to say a symbol is unknown or data is unavailable rather than filling the gap — the same "missing, never fabricated" invariant every engine in this codebase already follows. Carries the same no-advice rules and SEBI disclaimer language as `company_summary.py`'s prompt and this session's disclaimer pass.
- **Two real bugs found and fixed while building this, both live, both before shipping:**
  1. An optional-in-schema tool parameter (`run_screen`'s `industry`) had no Python default on the handler, so a model call that correctly omitted it crashed with a `TypeError` the model read as "industry is required" — it then spent its whole `MAX_TOOL_CALLS` budget guessing industry names before giving up. Fixed with a default, and guarded for the whole registry at once by a test that checks every schema-optional parameter has a matching handler default.
  2. Gemini attaches a `thoughtSignature` to a `functionCall` part and rejects the next turn with a 400 if the caller reconstructs that part from scratch instead of echoing it back verbatim. Fixed by having `GeminiChatProvider.step()` return the exact raw response part (`StepResult.raw_part`) for the caller to append unchanged, rather than rebuilding `{"functionCall": {...}}` from just the parsed name/args.
- **Not built:** persisted multi-turn chat history — this is stateless per question by design (a new conversation each call), matching the scope Screener.md §18 actually describes (standalone questions, not a saved thread). A chat-history table and thread UI are a natural v2, not attempted here.
- **API:** `POST /assistant/ask` (bare JSON `{question}` → `{answer, tools_used}`), gated behind `get_current_verified_user`, rate-limited tighter than `ai_summary` (10/day, not looser — one question can be several Gemini calls, not one).
- **UI:** `/research` page, `ResearchAssistantPanel` — a single-question input with example prompts, not a chat-thread interface; each answer shows which tools were actually called, for transparency.
- **Acceptance:** verified live end to end, three times, through the actual `ask()` function against the running container's real database and real Gemini keys — a company analysis question (score + historical falls, correctly citing real computed numbers), a screener discovery question (correctly reporting zero hits rather than fabricating some), and a personal-data question against an empty account (correctly reporting "no holdings, no watchlist" rather than erroring).
